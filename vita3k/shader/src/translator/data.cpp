...
bool USSETranslatorVisitor::vpck(
    ...
    // VPCK clears unwritten bytes in each packed 32-bit slot.
    ...
    // Internal registers store unpacked components.
    if (is_integer_data_type(inst.opr.dest.type) && type_size < 4 && inst.opr.dest.bank != RegisterBank::FPINTERNAL) {
