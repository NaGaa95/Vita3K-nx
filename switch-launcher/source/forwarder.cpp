...
constexpr const char *kLauncherNro = "sdmc:/switch/vita3k/Vita3K.nro";
...
Result ns_app_init()
{
    if(g_nsAppInitialized)return 0;
    g_nsAppSrv={};g_nsAppServiceOwned=false;
...
void ns_app_exit()
{
    if (!g_nsAppInitialized)
        return;
    if(g_nsAppServiceOwned)serviceClose(&g_nsAppSrv);
    nsExit();
    g_nsAppSrv = {};
...
{
    constexpr u64 maxContentSize=(UINT64_C(1)<<40)-1;
    ...
    for (size_t i = 0; i < ncas.size(); i++) {
        if(ncas[i].data.size()>maxContentSize)return {};
    ...
    std::vector<NcmContentInfo> contentInfos(ncas.size()+1);
    std::memcpy(&contentInfos[0].content_id,nca_entry.hash,sizeof(contentInfos[0].content_id));
    contentInfos[0].content_type=nca_entry.type;contentInfos[0].attr=0;
    ncmU64ToContentInfoSize(nca_entry.data.size(),&contentInfos[0]);contentInfos[0].id_offset=0;
    for(size_t index=0;index<packaged_content_info.size();index++)contentInfos[index+1]=packaged_content_info[index].info;
    BufHelper metadata;metadata.write(&out_header,sizeof(out_header));metadata.write(&cnmt_extended,sizeof(cnmt_extended));
    metadata.write(contentInfos.data(),contentInfos.size()*sizeof(contentInfos[0]));
    r.data=std::move(metadata.buf);r.contentIds.reserve(contentInfos.size());
    for(const NcmContentInfo &info:contentInfos)r.contentIds.push_back(info.content_id);
    return r;
}
...
bool patch_npdm(std::vector<u8> &npdm, u64 tid)
{
    ...
    // Kernel flags, four-core form: cpu ids 0..3 and a thread-priority ceiling
    // of 63, which is core 3's preemption priority (cores 0..2 use 59). Both
    // halves are required - granting the core without the ceiling leaves core 3
    // unable to preempt, which is what starves system services there. Written
    // here as well as in the manifest so a shortcut is correct even if the
    // prebuilt hbl.npdm in romfs was produced from an older one. The low nibble
    // marks the capability and npdm_patch_kc restores it.
    ...
    // Atmosphere 1.8+ requires bit 19 in both KAC sections. If the version cannot
    // be read we must not guess: a shortcut built with the wrong capability set
    // installs cleanly and then crashes the console when it is launched, so fail
    // the install here instead.
    u64 raw_ver = 0;
...
void copy_nacp_text(char *destination, size_t destinationSize, const std::string &source)
{
    ...
    size_t length = std::min(clean.size(), destinationSize - 1);
    while (length > 0 && length < clean.size() &&
...
bool readForwarderConfig(const std::string &path,std::string &nroPath,std::string &args)
{
    ...
    std::vector<char> data(static_cast<size_t>(st.st_size));FILE *file=fopen(path.c_str(),"rb");
    if(!file)
        return false;
    const bool ok=fread(data.data(),1,data.size(),file)==data.size()&&!ferror(file);
    if(fclose(file)!=0||!ok)return false;
    const auto first=std::find(data.begin(),data.end(),'\0');
    nroPath.assign(data.begin(),first);args.assign(first+1,data.end()-1);return true;
}
...
struct ForwarderConfigTransaction
{
    ...
    Result stage(u64 tid, const std::string &nroPath, const std::string &args)
    {
        ...
        if (mkdir("sdmc:/switch/vita3k/forwarders", 0777) != 0 &&
            (errno != EEXIST || stat("sdmc:/switch/vita3k/forwarders", &directoryStat) != 0 ||
             !S_ISDIR(directoryStat.st_mode)))
            return kForwarderIoError;

        char pathBuffer[128];
        const int length = snprintf(pathBuffer, sizeof(pathBuffer),
            "sdmc:/switch/vita3k/forwarders/%016llx.cfg", static_cast<unsigned long long>(tid));
    ...
    Result rollback()
    {
        bool ok = temp.empty() || remove(temp.c_str()) == 0 || errno == ENOENT;
        if (!installed)
            return ok ? 0 : kForwarderIoError;
        if (remove(path.c_str()) != 0 && errno != ENOENT)
            ok = false;
        if (hadPrevious && rename(backup.c_str(), path.c_str()) != 0)
            ok = false;
        installed = false;
        if (R_FAILED(fsdevCommitDevice("sdmc")))
            ok = false;
    ...
    {
        bool ok = (temp.empty() || remove(temp.c_str()) == 0 || errno == ENOENT) &&
                  (backup.empty() || remove(backup.c_str()) == 0 || errno == ENOENT);
        completed = true;
        installed = false;
        if (R_FAILED(fsdevCommitDevice("sdmc")))
            ok = false;
...
bool containsContentId(const std::vector<NcmContentId> &ids,const NcmContentId &id){
    return std::any_of(ids.begin(),ids.end(),[&](const NcmContentId &candidate){return contentIdsEqual(candidate,id);});
}

Result snapshotContentIds(NcmContentMetaDatabase &database,const NcmContentMetaKey &key,
                          const std::vector<u8> &metadata,std::vector<NcmContentId> &ids){
    if(metadata.size()<sizeof(NcmContentMetaHeader))return kForwarderIoError;
    NcmContentMetaHeader header{};std::memcpy(&header,metadata.data(),sizeof(header));
    const size_t prefix=sizeof(header)+header.extended_header_size;
    if(prefix<sizeof(header)||header.content_count>(SIZE_MAX-prefix)/sizeof(NcmContentInfo)||
       prefix+header.content_count*sizeof(NcmContentInfo)>metadata.size())return kForwarderIoError;
    ids.clear();if(!header.content_count)return 0;
    std::vector<NcmContentInfo> infos(header.content_count);s32 written=0;
    Result result=ncmContentMetaDatabaseListContentInfo(&database,&written,infos.data(),
    ...
    ids.reserve(infos.size());for(const NcmContentInfo &info:infos)ids.push_back(info.content_id);
    return 0;
}

Result snapshotContentMeta(NcmContentMetaDatabase &database,const NcmContentMetaKey &key,
                           ContentMetaBackup &backup){
    backup={};Result result=ncmContentMetaDatabaseHas(&database,&backup.existed,&key);
    if(R_FAILED(result)||!backup.existed)return result;
    u64 size=0;result=ncmContentMetaDatabaseGetSize(&database,&size,&key);
    if(R_SUCCEEDED(result)&&(size==0||size>1024*1024))result=kForwarderIoError;
    if(R_FAILED(result))return result;
    backup.data.resize(static_cast<size_t>(size));u64 received=0;
    result=ncmContentMetaDatabaseGet(&database,&key,&received,backup.data.data(),backup.data.size());
    if(R_SUCCEEDED(result)&&received!=backup.data.size())result=kForwarderIoError;
    if(R_SUCCEEDED(result))result=snapshotContentIds(database,key,backup.data,backup.contentIds);
    return result;
}
...
std::vector<NcmContentId> findReplacedContent(const std::vector<NcmContentId> &previous,
    ...
    std::vector<NcmContentId> replaced;for(const NcmContentId &id:previous)
      if(!containsContentId(current,id)&&!containsContentId(replaced,id))replaced.push_back(id);
    return replaced;
}

void removeOrphanedContent(NcmStorageId storageId,const std::vector<NcmContentId> &ids){
    if(ids.empty())return;
    NcmContentMetaDatabase database{};if(R_FAILED(ncmOpenContentMetaDatabase(&database,storageId)))return;
    NcmContentStorage storage{};if(R_FAILED(ncmOpenContentStorage(&storage,storageId))){ncmContentMetaDatabaseClose(&database);return;}
    for(const NcmContentId &id:ids){bool orphaned=false;
      if(R_FAILED(ncmContentMetaDatabaseLookupOrphanContent(&database,&orphaned,&id,1)))break;
      if(!orphaned)continue;
      bool present=false;
      if(R_FAILED(ncmContentStorageHas(&storage,&present,&id)))break;
      if(present&&R_FAILED(ncmContentStorageDelete(&storage,&id)))break;}
    ncmContentStorageClose(&storage);ncmContentMetaDatabaseClose(&database);
}
                         ...
                         const std::vector<u8> &iconJpeg, ForwarderStage &stage)
{
    ...
    const std::string args = (quotePath ? "\"" + nroPath + "\"" : nroPath) +
                             (gameKey=="launcher"?std::string{}:" -g \"" + gameKey + "\"");
    if (nroPath.empty() || nroPath.size() >= FS_MAX_PATH ||
        nroPath.size() + args.size() + 2 > 2046)
        return kForwarderIoError;

    u64 hashData[SHA256_HASH_SIZE / sizeof(u64)];
    const std::string stableHashSource="vita3k-forwarder:"+gameKey;
    sha256CalculateHash(hashData,stableHashSource.data(),stableHashSource.size());
...
const char *forwarderStageError(ForwarderStage stage)
{
    ...
    case ForwarderStage::Permissions: return "Could not read the Atmosphere version or patch the shortcut's permissions";
                      ...
                      const std::string &iconImgPath, char *err, std::size_t errSize)
{
    ...
    if (R_SUCCEEDED(rc)) {
        try {
            rc = install_forwarder(gameKey, name.empty() ? "Vita3K" : name,
...
bool forwarder_create_launcher(const std::string &iconImgPath,char *err,std::size_t errSize){
    return forwarder_create("launcher","Vita3K",iconImgPath,err,errSize);
}
