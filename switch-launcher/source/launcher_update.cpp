...
namespace
{
	constexpr char kLatestReleaseUrl[] =
		"https://api.github.com/repos/NaGaa95/Vita3K-nx/releases/latest";
	constexpr char kReleaseAssetPrefix[] =
		"https://github.com/NaGaa95/Vita3K-nx/releases/download/";
	constexpr char kDefaultLauncherPath[] = "sdmc:/switch/vita3k/Vita3K.nro";
	constexpr char kExpectedAssetName[] = "Vita3K.nro";
	constexpr std::size_t kMaxReleaseResponse = 2 * 1024 * 1024;
	constexpr std::uint64_t kMinimumNroSize = 1024 * 1024;
	constexpr std::uint64_t kMaximumNroSize = 1024ULL * 1024 * 1024;
	...
	std::atomic_bool s_installRequested{false};
	...
	void NotifyWake(bool throttle = false)
	{
		if (throttle)
		{
			...
			if (now - previous < 100 ||
				!s_lastProgressWakeMs.compare_exchange_strong(previous, now, std::memory_order_relaxed))
				return;
		}
		std::scoped_lock lock(s_wakeMutex);
		if (s_wakeCallback)
			s_wakeCallback(s_wakeUserData);
	}
	...
	void SetCommonCurlOptions(CURL* curl)
	{
		...
		curl_easy_setopt(curl, CURLOPT_USERAGENT, "Vita3K-nx-Updater/" VITA3K_NX_VERSION);
	...
	bool ReplaceLauncher(const std::string& launcherPath, const std::string& temporary, std::string& error)
	{
		...
		const auto restoreBackup = [&](const char* failure, const bool removeCurrent) {
			bool removed = true;
			if (removeCurrent && std::remove(launcherPath.c_str()) != 0 && errno != ENOENT)
				removed = false;
			const bool restored = removed && std::rename(backup.c_str(), launcherPath.c_str()) == 0;
			const bool committed = R_SUCCEEDED(fsdevCommitDevice("sdmc"));
			if (!restored)
			{
				error = std::string(failure) +
					" Automatic rollback also failed; the previous launcher remains in " + backup + ".";
			}
			else if (!committed)
			{
				error = std::string(failure) +
					" The previous launcher was restored, but that rollback could not be committed to SD.";
			}
			else
				error = failure;
			return false;
		...
		if (R_FAILED(fsdevCommitDevice("sdmc")))
			return restoreBackup("Could not commit the current launcher backup to the SD card.", false);
		if (std::rename(temporary.c_str(), launcherPath.c_str()) != 0)
			return restoreBackup("Could not activate the downloaded launcher.", false);
		if (R_FAILED(fsdevCommitDevice("sdmc")))
			return restoreBackup("Could not commit the launcher update to the SD card.", true);
		if (!ValidNro(launcherPath, 0))
			return restoreBackup("The installed launcher failed final validation.", true);
		return true;
	}
...
std::string LauncherUpdate_ResolveLauncherPath(const std::string& path)
{
	std::string candidate=path;
	if (StartsWith(candidate,"/switch/"))
		candidate.insert(0,"sdmc:");
	if (candidate.empty() || !StartsWith(candidate, "sdmc:/") || candidate.find('\\') != std::string::npos ||
		candidate.find("/../") != std::string::npos || candidate.find("/./") != std::string::npos ||
		(candidate.size() >= 3 && candidate.compare(candidate.size() - 3, 3, "/..") == 0) ||
		(candidate.size() >= 2 && candidate.compare(candidate.size() - 2, 2, "/.") == 0))
		return kDefaultLauncherPath;
	for (unsigned char value : candidate)
		if (value < 0x20 || value == 0x7f)
			return kDefaultLauncherPath;
	const std::string lower = Lower(candidate);
	if (lower.size() < 4 || lower.compare(lower.size() - 4, 4, ".nro") != 0)
		return kDefaultLauncherPath;
	return candidate;
}
...
void LauncherUpdate_RequestInstallation()
{
	s_installRequested.store(true, std::memory_order_release);
	NotifyWake();
}

bool LauncherUpdate_ConsumeInstallationRequest()
{
	return s_installRequested.exchange(false, std::memory_order_acq_rel);
}
...
bool LauncherUpdate_RecoverInstallation(const std::string& launcherPath, std::string& error)
{
	...
	const bool removedTemporary = lstat(temporary.c_str(), &currentStat) == 0;
	if (removedTemporary && std::remove(temporary.c_str()) != 0)
	{
	...
	if (!haveCurrent && haveBackup)
	{
		...
		if (R_FAILED(fsdevCommitDevice("sdmc")))
		{
			error = "Could not commit the restored launcher to the SD card.";
			return false;
		}
		return true;
	}
	if (haveCurrent && haveBackup)
	{
		...
		if (R_FAILED(fsdevCommitDevice("sdmc")))
		{
			error = "Could not commit launcher update recovery to the SD card.";
			return false;
		}
	}
	else if (removedTemporary && R_FAILED(fsdevCommitDevice("sdmc")))
	{
		error = "Could not commit incomplete update cleanup to the SD card.";
		return false;
	}
	return true;
}
