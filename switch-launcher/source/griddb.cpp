...
static int transfer_progress(void *userdata,curl_off_t,curl_off_t,curl_off_t,curl_off_t){
...
static bool http_get(const std::string &url, const std::string &bearer,
                     std::string &out, long *code,const std::atomic_bool *cancel=nullptr) {
  if (code) *code = 0;
  ...
  if(cancel){curl_easy_setopt(c,CURLOPT_NOPROGRESS,0L);curl_easy_setopt(c,CURLOPT_XFERINFOFUNCTION,transfer_progress);curl_easy_setopt(c,CURLOPT_XFERINFODATA,cancel);}
...
static bool http_download(const std::string &url, const std::string &path,const std::atomic_bool *cancel=nullptr) {
  std::string data;
  long code = 0;
  if (!http_get(url, "", data, &code,cancel) || code < 200 || code >= 300 || data.size() < 64)
    return false;
  ...
  if(!existed&&backupExists){
    if(rename(backup.c_str(),path.c_str())!=0||R_FAILED(fsdevCommitDevice("sdmc")))return false;
    existed=true;
  } else if(existed&&backupExists&&remove(backup.c_str())!=0)return false;
  ...
  if(existed&&rename(path.c_str(),backup.c_str())!=0){remove(temporary.c_str());return false;}
  ...
  if(existed){
    if(remove(backup.c_str())!=0||R_FAILED(fsdevCommitDevice("sdmc")))return false;
  }
  return true;
}
...
bool griddb_global_init(void) {
  ...
  // The launcher owns socket/curl lifetime so SMB mounts, updater workers,
  // firmware downloads, and SteamGridDB can safely coexist.
  g_networkReady = true;
  return true;
}
...
int griddb_search_games(const std::string &key, const std::string &title,
                        std::vector<GridDbGameResult> &results,const std::atomic_bool *cancel) {
  results.clear();
  if (key.empty()) return GRIDDB_NO_KEY;
  std::string response;
  long code = 0;
  std::string search = "https://www.steamgriddb.com/api/v2/search/autocomplete/" + url_encode(title);
  if (!http_get(search, key, response, &code,cancel)) return GRIDDB_NO_NET;
...
int griddb_fetch_artworks(const std::string &key, long gameId,
                          std::vector<GridDbArtwork> &artworks,const std::atomic_bool *cancel) {
  artworks.clear();
  if (key.empty()) return GRIDDB_NO_KEY;
  ...
  std::string response;
  long code = 0;
  if (!http_get(endpoint, key, response, &code,cancel)) return GRIDDB_NO_NET;
  int error = status_err(code);
  if (error) return error;
...
int griddb_download_image(const std::string &url, const std::string &outPath,const std::atomic_bool *cancel) {
  return !url.empty() && http_download(url, outPath,cancel) ? GRIDDB_OK : GRIDDB_ERROR;
}
...
int griddb_fetch_icons(const std::string &key, const std::string &title, const std::string &outDir,
                       int maxCount,const std::atomic_bool *cancel) {
