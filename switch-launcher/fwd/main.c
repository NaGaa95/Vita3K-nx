...
#define EXIT_DETECTION_STR "__VITA3K_FORWARDER_EXIT__"
#ifndef FWD_CFG_DIR
#define FWD_CFG_DIR "/switch/vita3k/forwarders/"
#endif
...
static const char g_noticeText[] = { "Vita3K-nx " VERSION };
...
// The manifest grants all four cores, but the process's initial thread is
// created against the default core and every thread the payload spawns
// inherits that mask. Widen it to whatever the kernel actually granted so the
// emulator can place its vblank, audio and render work on core 3. Failure is
// not fatal: the payload still runs on the cores it did get, and the launcher
// reports the count it is really allowed.
...
void NX_NORETURN loadNro(void) {
    ...
    // The sentinel means "leave the Application", whatever path accompanies it.
    // Vita3K sends its own return path, which is not the default NRO; falling
    // through to load that NRO fails with 0xD401 because Vita3K has already
    // exercised JIT and alias mappings in this process.
